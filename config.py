...
#tf_config['graph_options.rewrite_options.disable_meta_optimizer']   = True
...
random_seed = 10000                                          # Global random seed.
dataset     = EasyDict()                                    # Options for dataset.load_dataset().
train       = EasyDict(func='train.train_progressive_gan')  # Options for main training func.
G           = EasyDict(func='networks.G_paper')             # Options for generator network.
D           = EasyDict(func='networks.D_paper')             # Options for discriminator network.
G_opt       = EasyDict(beta_1=0.0, beta_2=0.99, epsilon=1e-8) # Options for generator optimizer.
D_opt       = EasyDict(beta_1=0.0, beta_2=0.99, epsilon=1e-8) # Options for discriminator optimizer.
...
# Dataset (choose one).
desc += '-sensat';            dataset = EasyDict(tfrecord_dir='sensat');
...
desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4}; sched.G_lrate_dict = {1024: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 18000
...
#desc += '-preset-v2-8gpus'; num_gpus = 8; sched.minibatch_base = 32; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8}; sched.G_lrate_dict = {512: 0.0015, 1024: 0.002}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
...
#train = EasyDict(func='util_scripts.generate_fake_images', run_id='networks\\23', num_pngs=4000); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
#train = EasyDict(func='util_scripts.generate_training_video', run_id=23, duration_sec=60.0); num_gpus = 1; desc = 'training-video-' + str(train.run_id)

#train = EasyDict(func='util_scripts.evaluate_metrics', run_id='networks\\23', log='metric-swd-16k.txt', metrics=['swd'], num_images=4000, real_passes=2); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id='networks\\23', log='metric-fid-10k.txt', metrics=['fid'], num_images=10000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
