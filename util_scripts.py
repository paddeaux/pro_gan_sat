...
tf.disable_v2_behavior()import scipy.ndimage
import scipy.misc
...
def generate_fake_images(run_id, snapshot=None, grid_size=[1, 1], num_pngs=1, image_shrink=1, png_prefix=None,
    ...
    for png_idx in range(num_pngs):
        ...
        images = Gs.run(latents, labels, minibatch_size=minibatch_size, num_gpus=config.num_gpus, out_mul=2048,
                        out_add=2048, out_shrink=image_shrink, out_dtype=np.uint16)
        misc.save_image_grid(images, os.path.join(result_subdir, '%s%06d.tif' % (png_prefix, png_idx)), [0, 4096],
                             grid_size)
...
def generate_interpolation_video(run_id, snapshot=None, grid_size=[1, 1], image_shrink=1, image_zoom=1,
                                 duration_sec=60.0, smoothing_sec=1.0, mp4=None, mp4_fps=30, mp4_codec='libx265',
                                 mp4_bitrate='16M', random_seed=1000, minibatch_size=8):
    network_pkl = misc.locate_network_pkl(run_id, snapshot)
    if mp4 is None:
...
def generate_training_video(run_id, duration_sec=20.0, time_warp=1.5, mp4=None, mp4_fps=30, mp4_codec='libx265',
    ...
    with open(os.path.join(src_result_subdir, 'log.txt'), 'rt') as log:
        for line in log:
            ...
            if k and l and t:
                ...
                png = os.path.join(src_result_subdir, 'fakes%06d.tif' % int(np.floor(k)))
    ...
    def make_frame(t):
        ...
        if png_cache[0] == png:
            ...
            all_bands = []
            img = gdal.Open(png)
            # reading red band as an array and appending to allbands
            all_bands.append(img.GetRasterBand(4).ReadAsArray())
            # reading green band as an array and appending to allbands
            all_bands.append(img.GetRasterBand(3).ReadAsArray())
            # reading blue band as an array and appending to allbands
            all_bands.append(img.GetRasterBand(2).ReadAsArray())
            # checking if all bands have same dimensions for sen12ms should be 256
            assert all_bands[0].shape == all_bands[1].shape == all_bands[2].shape
            # stack all the bands together
            rgb_composite = np.dstack((all_bands[0], all_bands[1], all_bands[2]))
            # resample the rgb array into 8 bits instead of 16 bits
            img = bytescale(rgb_composite, low=0, high=255)
