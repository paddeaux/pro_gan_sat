...
def train_progressive_gan(
    ...
    misc.save_image_grid(grid_reals, os.path.join(result_subdir, 'reals.tif'), drange=training_set.dynamic_range, grid_size=grid_size)
    misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.tif' % 0), drange=drange_net, grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=sched.minibatch//config.num_gpus)
                misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.tif' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
