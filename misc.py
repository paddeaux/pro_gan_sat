...
def convert_to_pil_image(image, drange=[0,1]):
    ...
    image = adjust_dynamic_range(image, drange, [0,4096])
    image = np.rint(image).clip(0, 4096).astype(np.uint16)
    format = 'RGB' if image.ndim == 3 else 'L'
    return PIL.Image.fromarray(image, format)

def convert_to_geo_image(image, filename, drange=[0,1]):
    image = adjust_dynamic_range(image, drange, [0,4096])
    image = np.rint(image).clip(0, 4096).astype(np.uint16)
    x_pixels = image.shape[2]  # number of pixels in x
    y_pixels = image.shape[1]  # number of pixels in y
    bands = image.shape[0]
    driver = gdal.GetDriverByName('GTiff')
    dataset = driver.Create(filename, x_pixels,
                            y_pixels, bands,gdal.GDT_Int16)
...
def save_image_grid(images, filename, drange=[0,1], grid_size=None):
    convert_to_geo_image(create_image_grid(images, grid_size), filename, drange)
