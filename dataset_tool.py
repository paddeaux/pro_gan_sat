...
class TFRecordExporter:
    ...
    def add_image(self, img):
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            ...
            quant = np.rint(img).clip(0, 4096).astype(np.uint16)
...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    img = gdal.Open(image_filenames[0])
    resolution = img.RasterXSize
    if img.RasterYSize != resolution:
        error('Input images must have the same width and height')
    if resolution != 2 ** int(np.floor(np.log2(resolution))):
        error('Input image resolution must be a power-of-two')

    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        for idx in range(np.arange(len(image_filenames)).size):
            img = gdal.Open(image_filenames[idx])
            #tfr.add_image(np.stack((img.GetRasterBand(4).ReadAsArray(), img.GetRasterBand(3).ReadAsArray(), img.GetRasterBand(2).ReadAsArray())))
            tfr.add_image(np.stack((img.GetRasterBand(1).ReadAsArray(), img.GetRasterBand(2).ReadAsArray(),
                                    img.GetRasterBand(3).ReadAsArray(), img.GetRasterBand(4).ReadAsArray(),
                                    img.GetRasterBand(5).ReadAsArray(), img.GetRasterBand(6).ReadAsArray(),
                                    img.GetRasterBand(7).ReadAsArray(), img.GetRasterBand(8).ReadAsArray(),
                                    img.GetRasterBand(9).ReadAsArray(), img.GetRasterBand(10).ReadAsArray(),
                                    img.GetRasterBand(11).ReadAsArray(), img.GetRasterBand(12).ReadAsArray(),
                                    img.GetRasterBand(13).ReadAsArray())))
